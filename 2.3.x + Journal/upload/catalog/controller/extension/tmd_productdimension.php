...
class ControllerExtensionTmdproductdimension extends Controller {
	...
	public function index() {
		...
		$data['textwidth']         = $this->language->get('textwidth');
		$data['textheight']        = $this->language->get('textheight');
	    ...
	    $status = $this->config->get('tmd_productdimension_status');
...
/* 19 06 2020 */
		...
		if(!empty($setting_info['width_min'])) {
			...
			$data['width_min'] 	= $this->config->get('tmd_productdimension_width_min');
		}

		if(!empty($setting_info['width_max'])) {
			$data['width_max'] = $setting_info['width_max'];
		} else {
			$data['width_max'] 	= $this->config->get('tmd_productdimension_width_max');
		}

		if(!empty($setting_info['height_min'])) {
			$data['height_min'] = $setting_info['height_min'];
		} else {
			$data['height_min'] = $this->config->get('tmd_productdimension_height_min');
		}

		if(!empty($setting_info['height_max'])) {
			$data['height_max'] = $setting_info['height_max'];
		} else {
			$data['height_max'] = $this->config->get('tmd_productdimension_height_max');
		}
/* 19 06 2020 */
		...
		$tmd_productdimension_h_color = $this->config->get('tmd_productdimension_h_color');
		...
		$tmd_productdimension_h_textcolor = $this->config->get('tmd_productdimension_h_textcolor');
	...
	public function quantityprice() {	
		...
		$statuss = $this->config->get('tmd_productdimension_status');
		if($statuss==1) {
			$tmdtext = $this->config->get('tmd_productdimension_multi');
