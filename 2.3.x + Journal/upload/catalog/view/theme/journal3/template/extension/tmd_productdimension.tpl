...
$( ".input_sqr" ).keyup(function() {
  ...
  if((width_min <= width && width_max >= width) && (height_min <= height && height_max >= height)){
    ...
    $(".tmder").remove();
      ...
      $('#productsqr').html(' ');
    $("#greater").after('<div class="alert alert-danger alert-dismissible"><?php echo $error_size; ?></div>');
  }
});
...
function lodprice(subprice){
    ...
    $('.product-price').html('<span style="text-decoration: line-through;">'+json['price']+'</span>');
    }
     else
    {
