<?php
namespace Opencart\Catalog\Model\Extension\Tmdproductmeasurement\Tmd;
class Tmdproductdimension extends \Opencart\System\Engine\Model {
   ...
   if ($product_id && $product_ids) {
		...
		if(!empty($query4->row['manufacturer_id'])) {
			...
			if ($manufacturer_id && $manufacturer_ids) {
			if (in_array((array)$manufacturer_id,(array)$manufacturer_ids)){
				$status=true;
			}
		}
		}
	...
	public function getTmdProductPricestatus($product_id) {
		...
		$categorie_ids = !empty($categorie_ids)?$categorie_ids:[];
		$manufacturer_ids = !empty($manufacturer_ids)?$manufacturer_ids:[];
